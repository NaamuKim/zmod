...
const fixtures = existsSync(fixturesRoot)
  ? readdirSync(fixturesRoot).filter((name) => {
      ...
      return (
        ...
        existsSync(join(dir, "codemod.ts"))
      );
    })
  : [];

describe("react codemod fixtures", () => {
  for (const fixture of fixtures) {
    it(`fixture: ${fixture}`, async () => {
      ...
      const codemod = await import(join(dir, "codemod.ts"));
      const result = transform(input, codemod.options);
